...
from humor.humor.utils.logging import Logger, mkdir
from humor.humor.utils.transforms import rotation_matrix_to_angle_axis, batch_rodrigues

from humor.humor.datasets.amass_utils import CONTACT_INDS

from humor.humor.body_model.utils import SMPL_JOINTS, KEYPT_VERTS, smpl_to_openpose

from humor.humor.fitting.fitting_utils import OP_IGNORE_JOINTS, parse_floor_plane, compute_cam2prior, OP_EDGE_LIST, log_cur_stats
from humor.humor.fitting.fitting_loss import FittingLoss
...
class MotionOptimizer():
    ...
    def infer_latent_motion(self, trans, root_orient, body_pose, betas, data_fps, full_forward_pass=False):
        ...
        '''
        print("infer latent motion BEGIN")
        print("-------------------------")
        print("shape of trans: {}".format(trans.shape))
        print("shape of root_orient: {}".format(root_orient.shape)) 
        print("shape of body_pose: {}".format(body_pose.shape))
    ...
    def rollout_latent_motion(self, trans, root_orient, body_pose, betas, prior_opt_params, latent_motion,
        ...
        '''
        print("rollout latent motion BEGIN")
        print("-------------------------")
        print("shape of trans {}".format(trans.shape))
        print("shape of root_orient {}".format(root_orient.shape))
        print("shape of body_pose {}".format(body_pose.shape))
        print("shape of betas {}".format(betas.shape))
        print("shape of prior_opt_params {}".format(prior_opt_params[0].shape))
        print("shape of latent_motion {}".format(latent_motion[0].shape))
        ...
        if self.motion_prior.model_data_config in ['smpl+joints', 'smpl+joints+contacts']:
            ...
            smpl_results, _ = self.smpl_results(trans, root_orient, body_pose, betas)
            joints = smpl_results['joints3d']
            print('shape of joints {}'.format(joints.shape))
            # update to correct rotations for input
            root_orient_in = root_orient 
            print('shape of root_orient_in {}'.format(root_orient_in.shape))
            body_pose_in = body_pose
            print('shape of body_pose_in {}'.format(body_pose_in.shape))
            # if self.motion_prior.in_rot_rep == 'mat' or self.motion_prior.in_rot_rep == '6d':
            ...
            joints_in = joints.reshape((B, joints.shape[1], -1))
            joints_vel_in = joints_vel.reshape((B,joints.shape[1], -1))
