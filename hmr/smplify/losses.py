...
def camera_fitting_loss(model_joints, camera_t, camera_t_est, camera_center, joints_2d, joints_conf,
    ...
    # if gt keypoints are all empty, then definitely use op's
    if joints_conf[:, gt_joints_ind].sum().item() == 0:
        is_valid = (joints_conf[:, op_joints_ind].max(dim=-1)[0][:,None,None] > 0).float()
