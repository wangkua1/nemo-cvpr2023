...
def main(args):
    ...
    if args.input_image_folder == '':
        video_file = args.vid_file

        # ========= [Optional] download the youtube video ========= #
        if video_file.startswith('https://www.youtube.com'):
            print(f'Donwloading YouTube video \"{video_file}\"')
            video_file = download_youtube_clip(video_file, '/scratch/users/wangkua1/data/youtube_clips')
        ...
        img_shape = cv2.imread(osp.join(image_folder, image_names[0])).shape

        # Make output directory
        base_dir, dirname = osp.split(args.input_image_folder)
        output_path = os.path.join(args.output_folder, dirname)
        os.makedirs(output_path, exist_ok=True)
    ...
    if args.tracking_method == 'pose':
        if video_file == '':
            def penn_images_to_video(img_folder, output_vid_file):
            ...
            video_file = osp.join(output_path, 'video.mp4') 
            penn_images_to_video(args.input_image_folder, video_file)
    ...
    for person_id in tqdm(list(tracking_results.keys())):
        ...
        with torch.no_grad():
            ...
            for batch in dataloader:
                if has_keypoints:
                    batch, nj2d = batch
                    norm_joints2d.append(nj2d.numpy().reshape(-1, 25, 3)[:, :21])
        ...
        if args.run_smplify and args.tracking_method == 'pose':
            ...
            # norm_joints2d = convert_kps(norm_joints2d, src='insta', dst='spin')  # since i'm using body25 model
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_image_folder', type=str, default='',
                        help='input video path or youtube link')
