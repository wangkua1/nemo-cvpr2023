...
for n in 1 2 3 4 1_1 1_2 1_3 2_1; do
...
expname=${cur_fname}-${action}-n${n}-${lr_human}-${lr_instance}-${instance_code_size}-${lr_phase}-${weight_gmm_loss}-${weight_vp_loss}-${weight_vp_z_loss}-${h_dim}-${phase_rbf_dim}-${rbf_kernel}-${warmup_step}-${lr_factor}-${loss}-${label_type}-${weight_3d_loss}

partition=pasteur

cmd="python -m scripts.learned_multi_view_recon_nn \
    --default_config config/20221008-default-v1.yml \
    --label_type ${label_type} \
    --data_loader_type generic \
    --nemo_cfg_path nemo/config/nemo_ablations_jeff/${action}_n${n}.yml \
    --out_dir shared_out/jeff_out/${expname} \
    --lr_factor ${lr_factor} \
    --batch_size 512 \
    --n_steps ${n_steps} \
